...
//! evaluation. The inner kernel is explicit SIMD ([`crate::quasistatic::simd::p2p_accum`],
...
/// which is what the GPU runs; see `crate::quasistatic::gpu::Precision`.
pub fn p2p_laplace(
    tx: &[f64],
    ty: &[f64],
    tz: &[f64],
    ...
    out.par_iter_mut().enumerate().for_each(|(i, oi)| {
        *oi = crate::quasistatic::simd::p2p_accum(tx[i], ty[i], tz[i], src.x, src.y, src.z, src.q);
    });
}
