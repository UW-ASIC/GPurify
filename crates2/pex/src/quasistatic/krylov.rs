...
use crate::quasistatic::simd::{dot, nrm2};
...
pub fn gmres(
    ...
    for _cycle in 0..opts.max_restarts {
        ...
        for k in 0..m {
            ...
            for i in 0..=k {
                h[i][k] = dot(&w, &v[i]);
                crate::quasistatic::simd::axpy(-h[i][k], &v[i], &mut w);
            }
            h[k + 1][k] = nrm2(&w);
...
use crate::quasistatic::kernel::ComplexOperator;
    ...
    for _cycle in 0..opts.max_restarts {
        ...
        for k in 0..m {
            ...
            for i in 0..=k {
                ...
                let (hre, him) = crate::quasistatic::simd::cdot_conj(vr, vi, &wre, &wim);
                h[i][k] = Complex64::new(hre, him);
                // w -= h·v_i  (complex product in split form)
                crate::quasistatic::simd::caxpy(hre, him, vr, vi, &mut wre, &mut wim);
            }
