...
use gdsverify_pex::quasistatic::integrals::filament::self_inductance_bar;
...
fn dc_resistance_is_exact() {
    ...
    let res = gdsverify_pex::quasistatic::henry::run(&deck).unwrap();
...
fn subdivided_inductance_close_to_single_bar() {
    ...
    let f = 1.0e3;
    let single = {
        let res = gdsverify_pex::quasistatic::henry::run(&wire_deck(1, 1, f)).unwrap();
        let w = 2.0 * std::f64::consts::PI * f;
        res.z[0][(0, 0)].im / w
    };
    let sub = {
        let res = gdsverify_pex::quasistatic::henry::run(&wire_deck(4, 4, f)).unwrap();
        let w = 2.0 * std::f64::consts::PI * f;
        res.z[0][(0, 0)].im / w
    };
...
fn skin_effect_raises_ac_resistance() {
    ...
    let r_dc = gdsverify_pex::quasistatic::henry::run(&wire_deck(6, 6, 1.0)).unwrap().z[0][(0, 0)].re;
    let r_ac = gdsverify_pex::quasistatic::henry::run(&wire_deck(6, 6, 1.0e9)).unwrap().z[0][(0, 0)].re;
...
fn iterative_matches_direct() {
    ...
    let deck = "\
        ...
        .freq fmin=1e8 fmax=1e8 ndec=1\n\
        .end\n";
    let nl = gdsverify_pex::quasistatic::henry::parse(deck).unwrap();
    let zd = gdsverify_pex::quasistatic::henry::solve_with(&nl, gdsverify_pex::quasistatic::henry::Method::Direct).unwrap();
    let zi = gdsverify_pex::quasistatic::henry::solve_with(&nl, gdsverify_pex::quasistatic::henry::Method::Iterative).unwrap();
...
fn equiv_merges_nodes() {
    ...
    let equiv = "\
        ...
        .freq fmin=1e3 fmax=1e3 ndec=1\n\
        .end\n";
    let za = gdsverify_pex::quasistatic::henry::run(joined).unwrap().z[0][(0, 0)];
    let zb = gdsverify_pex::quasistatic::henry::run(equiv).unwrap().z[0][(0, 0)];
...
fn ground_plane_reduces_inductance_and_matches_image_loop() {
    ...
    let freq = 1.0e3;
    let ind = |deck: &str| {
        let r = gdsverify_pex::quasistatic::henry::run(deck).unwrap();
...
fn mesh_formulation_matches_nodal() {
    ...
    for (di, deck) in decks.iter().enumerate() {
        let nl = gdsverify_pex::quasistatic::henry::parse(deck).unwrap();
        let zn = gdsverify_pex::quasistatic::henry::solve_full(&nl, gdsverify_pex::quasistatic::henry::Method::Direct, gdsverify_pex::quasistatic::henry::Formulation::Nodal).unwrap();
        let zm = gdsverify_pex::quasistatic::henry::solve_full(&nl, gdsverify_pex::quasistatic::henry::Method::Direct, gdsverify_pex::quasistatic::henry::Formulation::Mesh).unwrap();
