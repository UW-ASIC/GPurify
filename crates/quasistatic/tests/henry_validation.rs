...
fn dc_resistance_is_exact() {
    ...
    let res = gpurify_quasistatic::henry::run(&deck).unwrap();
    let r = res.z[0][(0, 0)].re;
    let expected = 0.1 / (5.8e7 * 0.001 * 0.001);
    let rel = (r - expected).abs() / expected;
    println!("R = {r:.6e} Ω, expected {expected:.6e} Ω, rel {rel:.2e}");
...
fn single_filament_self_inductance_matches_bar_formula() {
    ...
    println!("L = {l:.6e} H, bar formula {expected:.6e} H, rel {rel:.2e}");
...
fn subdivided_inductance_close_to_single_bar() {
    ...
    let f = 1.0e3;
    let single = {
        let res = gpurify_quasistatic::henry::run(&wire_deck(1, 1, f)).unwrap();
        let w = 2.0 * std::f64::consts::PI * f;
        res.z[0][(0, 0)].im / w
    };
    let sub = {
        let res = gpurify_quasistatic::henry::run(&wire_deck(4, 4, f)).unwrap();
        let w = 2.0 * std::f64::consts::PI * f;
        res.z[0][(0, 0)].im / w
    };
    let rel = (single - sub).abs() / single;
    println!(
        "L single = {single:.6e} H, L subdivided = {sub:.6e} H, rel {:.3}%",
        rel * 100.0
    );
    assert!(
        rel < 0.05,
        "subdivided inductance drifted {:.2}% from single bar",
        rel * 100.0
    );
}
...
fn skin_effect_raises_ac_resistance() {
    ...
    let r_dc = gpurify_quasistatic::henry::run(&wire_deck(6, 6, 1.0))
        .unwrap()
        .z[0][(0, 0)]
        .re;
    let r_ac = gpurify_quasistatic::henry::run(&wire_deck(6, 6, 1.0e9))
        .unwrap()
        .z[0][(0, 0)]
        .re;
    println!(
        "R_dc = {r_dc:.6e} Ω, R_ac(1GHz) = {r_ac:.6e} Ω, ratio {:.3}",
        r_ac / r_dc
    );
...
fn impedance_matrix_is_symmetric() {
    ...
    println!("Z01 = {}, Z10 = {}, asym {asym:.2e}", z[(0, 1)], z[(1, 0)]);
...
fn iterative_matches_direct() {
    ...
    let deck = "\
        ...
        .freq fmin=1e8 fmax=1e8 ndec=1\n\
        .end\n";
    let nl = gpurify_quasistatic::henry::parse(deck).unwrap();
    let zd =
        gpurify_quasistatic::henry::solve_with(&nl, gpurify_quasistatic::henry::Method::Direct)
            .unwrap();
    let zi =
        gpurify_quasistatic::henry::solve_with(&nl, gpurify_quasistatic::henry::Method::Iterative)
            .unwrap();
...
fn equiv_merges_nodes() {
    ...
    let equiv = "\
        ...
        .freq fmin=1e3 fmax=1e3 ndec=1\n\
        .end\n";
    let za = gpurify_quasistatic::henry::run(joined).unwrap().z[0][(0, 0)];
    let zb = gpurify_quasistatic::henry::run(equiv).unwrap().z[0][(0, 0)];
...
fn ground_plane_reduces_inductance_and_matches_image_loop() {
    ...
    let freq = 1.0e3;
    let ind = |deck: &str| {
        let r = gpurify_quasistatic::henry::run(deck).unwrap();
    ...
    println!(
        "L_alone={l_alone:.4e} H, L_over_plane={l_plane:.4e} H, L_loop={l_loop:.4e} H, L_loop/2={:.4e}",
        l_loop / 2.0
    );
    ...
    println!("image vs explicit-loop rel = {rel:.3}");
    assert!(
        rel < 0.10,
        "image ground plane disagrees with explicit loop: {rel}"
    );
}
...
fn mesh_formulation_matches_nodal() {
    ...
    for (di, deck) in decks.iter().enumerate() {
        let nl = gpurify_quasistatic::henry::parse(deck).unwrap();
        let zn = gpurify_quasistatic::henry::solve_full(
            &nl,
            gpurify_quasistatic::henry::Method::Direct,
            gpurify_quasistatic::henry::Formulation::Nodal,
        )
        .unwrap();
        let zm = gpurify_quasistatic::henry::solve_full(
            &nl,
            gpurify_quasistatic::henry::Method::Direct,
            gpurify_quasistatic::henry::Formulation::Mesh,
        )
        .unwrap();
        ...
        assert!(
            maxrel < 1e-9,
            "mesh formulation disagrees with nodal (deck {di}): {maxrel}"
        );
    }
}

/// docs/CONVENTIONS.md bit-identity law (see `krylov::determinism_tests`): the
/// same henry solve must produce byte-identical impedances on repeat runs and
/// at any rayon thread count.
#[test]
fn henry_solve_is_bit_identical_across_runs_and_thread_counts() {
    // Iterative path (nhinc/nwinc bundle) so the rayon column solves and the
    // GMRES inner folds are all exercised.
    fn solve_bits(threads: usize) -> Vec<(u64, u64)> {
        ...
        pool.install(|| {
            let deck = "\
                ...
                N3 x=0 y=0.002 z=0\n\
                N4 x=0.05 y=0.002 z=0\n\
                E1 N1 N2 w=0.0002 h=0.0002 sigma=5.8e7 nhinc=3 nwinc=3\n\
                E2 N3 N4 w=0.0002 h=0.0002 sigma=5.8e7 nhinc=3 nwinc=3\n\
                .external N1 N2 porta\n\
                .external N3 N4 portb\n\
                .freq fmin=1e6 fmax=1e8 ndec=1\n\
                .end\n";
            let nl = gpurify_quasistatic::henry::parse(deck).unwrap();
            let res = gpurify_quasistatic::henry::solve_with(
                &nl,
                gpurify_quasistatic::henry::Method::Iterative,
            )
            .unwrap();
            res.z
                .iter()
                .flat_map(|z| z.data.iter().map(|c| (c.re.to_bits(), c.im.to_bits())))
                .collect()
        })
    }
