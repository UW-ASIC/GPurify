...
fn eval_eq(x: f64, y: f64, z: f64, ref_len: f64) -> f64 {
    ...
    let nz = |v: f64| usize::from(v.abs() < BRICK_EPS);
...
mod tests {
    ...
    #[expect(
        clippy::float_cmp,
        reason = "û₁·û₂ is exactly 0.0, so the quadrature returns an exact 0.0"
    )]
    ...
    fn touching_elbow_quadrature_accuracy() {
        ...
        for i in 0..nsub {
            let a1 = f1.a + d1 * (f64::from(i) / f64::from(nsub));
            let b1 = f1.a + d1 * (f64::from(i + 1) / f64::from(nsub));
            for j in 0..nsub {
                let a2 = f2.a + d2 * (f64::from(j) / f64::from(nsub));
                let b2 = f2.a + d2 * (f64::from(j + 1) / f64::from(nsub));
    ...
    fn brick_matches_cross_section_subdivision() {
        ...
        for i1 in 0..nsub {
            for j1 in 0..nsub {
                let o1 = wd * ((f64::from(i1) + 0.5) / f64::from(nsub) - 0.5) * w
                    + hd * ((f64::from(j1) + 0.5) / f64::from(nsub) - 0.5) * t;
                for i2 in 0..nsub {
                    for j2 in 0..nsub {
                        let o2 = wd * ((f64::from(i2) + 0.5) / f64::from(nsub) - 0.5) * w
                            + hd * ((f64::from(j2) + 0.5) / f64::from(nsub) - 0.5) * t;
        ...
        refv /= f64::from(nsub * nsub * nsub * nsub);
