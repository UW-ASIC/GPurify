...
// ---------------------------------------------------------------------------
//
// ponytail: the CubeCL/CUDA `gpu_kernel` module (dev_hash_k etc.) and the
// feature-gated GPU dispatch were removed. `compute_dev_hashes` is now the host
// path directly; the GPU dev-hash kernel is staged for phase-2 vulkano. The
// host math is byte-for-byte what the old GPU path fell back to, so every
// CPU==GPU equality test still holds.
