...
use quasiss::integrals::filament::{
    self, mutual_parallel, mutual_parallel_equal_grover, Filament,
};
use quasiss::integrals::panel::potential_integral;
...
use quasiss::linalg::DenseMatrix;
use quasiss::operator::{BlockJacobiComplex, DenseComplexOperator};
use quasiss::krylov::{gmres_complex, GmresOptions};
...
fn complex_gmres_matches_dense_lu() {
    ...
    let lu = quasiss::linalg::LuDecomposition::factor(&a).unwrap();
    let x_lu = lu.solve(&b);
...
fn iterative_refinement_recovers_f64_from_f32() {
    use quasiss::refine::{solve_iterative_refinement, solve_equilibrated, RefineOptions};
