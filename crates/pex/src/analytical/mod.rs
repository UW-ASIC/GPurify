...
/// Typed error for the fail-closed per-net entry points.
...
// --- Rule plumbing: one extractor per file in rules/ -------------------------
...
pub struct PexCtx<'a> {
    ...
    /// Requested backend. The analytical path is exact on CPU; this is carried
    /// only so a future prefilter can consult it (the old device session is gone).
    pub backend: Backend,
}
...
pub mod rules {
    //! The analytical rule set, one file per extractor.
    use super::{BoxedRule, PexCtx};
    ...
    pub type Factory = fn(LayerId, &PexLayerParams) -> Option<BoxedRule>;

    // ponytail: explicit list, not a build.rs glob — four rules, adding one is a
    // two-line diff and this drops the build script + OUT_DIR include entirely.
    #[path = "area_fringe_cap.rs"]
    pub mod area_fringe_cap;
    #[path = "coupling_cap.rs"]
    pub mod coupling_cap;
    #[path = "resistance.rs"]
    pub mod resistance;
    #[path = "via_resistance.rs"]
    pub mod via_resistance;

    /// Every rule in this engine's rules/ folder.
    pub static FACTORIES: &[Factory] = &[
        area_fringe_cap::FACTORY,
        coupling_cap::FACTORY,
        resistance::FACTORY,
        via_resistance::FACTORY,
    ];

    // Silence unused-import warnings on the trait-object context aliases when a
    // rule file imports them for its `impl Rule<PexCtx>` blocks.
    #[allow(unused_imports)]
    use super::Attributed as _;
    #[allow(unused_imports)]
    use PexCtx as _;
}
...
///
/// The analytical models are exact on CPU; a requested [`Backend::Gpu`] is
/// honored in telemetry but still runs the CPU rule logic (the device session /
/// prefilter is gone). The `Result` shape is kept for API compatibility.
pub fn run_pex_backend(
    store: &GeometryStore,
    deck: &Deck,
    backend: Backend,
) -> Result<(PexReport, crate::backend::BackendTelemetry), PexError> {
    let parasitics = extract_all(store, deck, backend)
        .into_iter()
        .map(|(p, _)| p)
        .collect();
    // Analytical path always runs on CPU regardless of the request.
    let telemetry = crate::backend::BackendTelemetry::new(backend, Backend::Cpu);
    Ok((PexReport { parasitics }, telemetry))
}
...
/// Unattributed/floating geometry is retained under the sentinel net `u32::MAX`,
/// so [`PexReport::floating_metal_cap`] can report it rather than silently
/// losing it.
///
...
pub fn run_pex_by_net_checked(
    ...
    if diagnostics.is_empty() {
        ...
        Err(PexError::UnsupportedGeometry(diagnostics))
    }
}
...
pub(crate) struct RectilinearMetrics {
    pub(crate) area_nm2: f64,
    pub(crate) perimeter_nm: f64,
    pub(crate) equivalent_length_nm: f64,
    pub(crate) equivalent_width_nm: f64,
}
...
pub(crate) fn rectilinear_metrics(store: &GeometryStore, poly: PolyId) -> Result<RectilinearMetrics, String> {
...
pub(crate) fn report_dimension_nm(value: f64) -> i32 {
    value.round().clamp(1.0, i32::MAX as f64) as i32
}

pub(crate) fn extraction_diagnostic(
    lt: &LayerTable,
    layer: LayerId,
    poly: PolyId,
    model: &str,
...
mod tests {
    ...
    fn unsupported_geometry_is_reported_instead_of_silently_zeroed() {
        ...
        match run_pex_by_net_checked(&st, &deck, &[0]).unwrap_err() {
            PexError::UnsupportedGeometry(diags) => assert_eq!(diags.len(), 2),
        }
    }
}
