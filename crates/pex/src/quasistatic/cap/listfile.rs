...
//! read by [`crate::quasistatic::cap::geometry::parse`]. A trailing `-` flag flips the stored panel
//! orientation (rarely needed for closed surfaces).

use crate::quasistatic::cap::dielectric::{PanelRole, Problem};
use crate::quasistatic::cap::geometry::parse as parse_qui;
        ...
        match kind {
            "C" | "c" => {
                ...
                for panel in geo.panels {
                    ...
                    problem.panels.push(crate::quasistatic::integrals::panel::Panel::new(verts, gid));
            ...
            "D" | "d" => {
                ...
                for panel in geo.panels {
                    let verts = panel.vertices.iter().map(|v| *v + tr).collect();
                    problem.panels.push(crate::quasistatic::integrals::panel::Panel::new(verts, 0));
...
fn load_geo(base_dir: &Path, file: &str, lineno: usize) -> Result<crate::quasistatic::cap::geometry::Geometry, ListError> {
