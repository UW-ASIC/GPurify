...
use crate::quasistatic::henry::netlist::{GroundPlane, Netlist};
use crate::quasistatic::geometry::Vec3;
use crate::quasistatic::integrals::filament::{self, Filament};
use crate::quasistatic::krylov::{gmres_complex, GmresOptions};
use crate::quasistatic::linalg::{DenseMatrix, LuDecomposition};
use crate::quasistatic::operator::{BlockJacobiComplex, DenseComplexOperator};
...
#[cfg(feature = "gpu")]
type GpuOpt = Option<std::sync::Arc<crate::quasistatic::gpu::VulkanoBackend>>;
#[cfg(not(feature = "gpu"))]
...
#[cfg(feature = "gpu")]
use crate::quasistatic::gpu::op::GpuZbOp;
...
use crate::quasistatic::integrals::filament::default_wdir;
...
pub fn solve_gpu(
    nl: &Netlist,
    gpu: std::sync::Arc<crate::quasistatic::gpu::VulkanoBackend>,
...
fn solve_impl(nl: &Netlist, method: Method, formulation: Formulation, gpu: GpuOpt) -> Result<SolveResult, SolveError> {
    ...
    let l_handle = gpu.as_ref().map(|g| {
        use crate::quasistatic::gpu::ComputeBackend;
        g.upload_matrix(&crate::quasistatic::gpu::DeviceMatrix::from_rows(nb, nb, l.data.clone()))
    });
    ...
    for &f in &freqs {
        ...
        if formulation == Formulation::Mesh {
            let zmat = crate::quasistatic::henry::mesh_analysis::solve_ports(&zb, &edges, &port_pairs, nn);
            z_all.push(zmat);
            continue;
        }
        ...
        let gpu_xcols: Option<Vec<Option<Vec<Complex64>>>> =
            gpu_parts.as_ref().map(|(op, precond)| {
                ...
                let sols = crate::quasistatic::krylov::gmres_complex_batched(
                    |x: &[Complex64], y: &mut [Complex64], ncols: usize| {
                        op.apply_batch(x, y, ncols)
                    },
                    &rhs_cols,
                    Some(precond as &(dyn crate::quasistatic::kernel::ComplexOperator + Sync)),
                    opts,
                );
