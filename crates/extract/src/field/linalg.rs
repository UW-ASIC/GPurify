...
impl<T: Field> LuDecomposition<T> {
    ...
    pub fn factor(a: &DenseMatrix<T>) -> Result<Self, LinalgError> {
        ...
        for k in 0..n {
            ...
            // Determinism: each parallel task owns one disjoint row and its
            // arithmetic is a strict sequential loop — no cross-row reduction,
...
impl LowRankApprox {
    ...
    pub fn compress(a: &[f64], m: usize, n: usize, eps: f64) -> Self {
        ...
        let max_k = m.min(n);
        let mut k = 0;
        let norm_threshold = eps * col_norms.iter().copied().fold(0.0f64, f64::max);
...
#[inline]
pub(crate) fn cdot(ar: &[f64], ai: &[f64], xr: &[f64], xi: &[f64]) -> (f64, f64) {
...
#[inline]
pub(crate) fn cdot_conj(vr: &[f64], vi: &[f64], wr: &[f64], wi: &[f64]) -> (f64, f64) {
...
#[inline]
pub(crate) fn caxpy(hre: f64, him: f64, vr: &[f64], vi: &[f64], wr: &mut [f64], wi: &mut [f64]) {
    for j in 0..vr.len() {
