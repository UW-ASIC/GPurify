...
//!
//! Determinism: the `HashMap`s here (`node_index`, key=value token maps) are
//! lookup-only — every assembly-facing collection (`nodes`, `segments`,
//! `ports`, `equivs`) is a `Vec` in deck order, and no map is ever iterated.

use crate::field::henry::units::units_to_meters;
...
impl FreqSweep {
    ...
    pub fn frequencies(&self) -> Vec<f64> {
        ...
        #[expect(
            ...
            reason = "point count is small, positive, and rounded before the cast"
        )]
        let n = (decades * self.ndec).round().max(1.0) as usize;
        (0..=n)
            .map(|i| {
                #[expect(clippy::cast_precision_loss, reason = "sweep index is a small count")]
                let e = i as f64;
                self.fmin * 10f64.powf(e / self.ndec)
            })
            .collect()
    }
}
...
#[expect(
    clippy::cast_possible_truncation,
    clippy::cast_sign_loss,
    reason = "nhinc/nwinc are small user-supplied counts, clamped to >= 1 downstream"
)]
fn get_count(m: &HashMap<String, String>, k: &str) -> Option<usize> {
    getf(m, k).map(|x| x as usize)
}
...
pub fn parse(input: &str) -> Result<Netlist, ParseError> {
    ...
    for (lineno0, raw) in input.lines().enumerate() {
        ...
        if lower.starts_with(".units") {
            let u = tokens
                .get(1)
                .ok_or_else(|| ParseError::At(lineno, "missing unit".into()))?;
            ...
            if let Some(n) = get_count(&m, "nhinc") {
                def_nhinc = n;
            }
            if let Some(n) = get_count(&m, "nwinc") {
                def_nwinc = n;
            }
            ...
            let n1 = tokens
                .get(1)
                .ok_or_else(|| ParseError::At(lineno, "external: missing node1".into()))?;
            let n2 = tokens
                .get(2)
                .ok_or_else(|| ParseError::At(lineno, "external: missing node2".into()))?;
            let name = tokens.get(3).map_or_else(
                || format!("port{}", nl.ports.len() + 1),
                ToString::to_string,
            );
            nl.ports.push(Port {
                name,
            ...
            let group: Vec<String> = tokens[1..].iter().map(ToString::to_string).collect();
            if group.len() >= 2 {
            ...
            nl.ground_plane = Some(GroundPlane {
                point: [px, py, pz],
                normal: [nx, ny, nz],
            });
        } else if lower.starts_with(".freq") {
            let m = kv(&tokens[1..]);
            let fmin = getf(&m, "fmin")
                .ok_or_else(|| ParseError::At(lineno, "freq: missing fmin".into()))?;
            ...
            let w = getf(&m, "w")
                .or(def_w)
                .ok_or_else(|| ParseError::At(lineno, "segment: missing w".into()))?
                * scale;
            let h = getf(&m, "h")
                .or(def_h)
                .ok_or_else(|| ParseError::At(lineno, "segment: missing h".into()))?
                * scale;
            ...
            let nhinc = get_count(&m, "nhinc").unwrap_or(def_nhinc).max(1);
            let nwinc = get_count(&m, "nwinc").unwrap_or(def_nwinc).max(1);
