...
//! `LinearOperator`/GMRES path is the drop-in accelerated alternative for
//! large B.
//!
//! Determinism: filaments are emitted in segment-vector order (deck order),
//! the inductance rows are a rayon indexed collect (order-preserving), and
//! every reduction is a strict sequential fold — bit-identical across runs and
//! thread counts.

#![expect(
    clippy::needless_range_loop,
    reason = "ported assembly kernels index several SoA lanes in lockstep; ranged loops keep the strict fold order explicit"
)]

use crate::field::geometry::Vec3;
use crate::field::henry::netlist::{GroundPlane, Netlist};
use crate::field::integrals::filament::{self, default_wdir, Filament};
use crate::field::krylov::{gmres_complex, GmresOptions};
use crate::field::linalg::{DenseMatrix, LuDecomposition};
...
/// Discretize all segments into branch filaments (`SoA` lanes), in
/// segment-vector (deck) order.
fn build_filaments(nl: &Netlist) -> Result<FilamentSet, SolveError> {
    let mut set = FilamentSet {
        fils: Vec::new(),
        r: Vec::new(),
        n1: Vec::new(),
        n2: Vec::new(),
    ...
    for seg in &nl.segments {
        let i1 = nl
            .node_idx(&seg.n1)
            .ok_or_else(|| SolveError::UnknownNode(seg.n1.clone()))?;
        let i2 = nl
            .node_idx(&seg.n2)
            .ok_or_else(|| SolveError::UnknownNode(seg.n2.clone()))?;
        ...
        #[expect(
            ...
            reason = "node counts are far below u32::MAX; indices come from a Vec"
        )]
        let (i1_32, i2_32) = (i1 as u32, i2 as u32);
        for &(off_w, sub_w) in &wparts {
            for &(off_h, sub_h) in &hparts {
                ...
                set.fils
                    .push(Filament::with_frame(a, b, sub_w, sub_h, wdir));
                set.r.push(r_sub);
                set.n1.push(i1_32);
                set.n2.push(i2_32);
            }
    ...
    let rel: Vec<f64> = (0..n)
        .map(|i| {
            #[expect(
                ...
                reason = "distance to the nearer edge of a small filament count"
            )]
...
fn mirror_filament(f: &Filament, p0: Vec3, n: Vec3) -> Filament {
    ...
    Filament::with_frame(
        mirror_point(f.a, p0, n),
        mirror_point(f.b, p0, n),
        f.w,
        f.t,
        wdir,
    )
...
/// Total branch (filament) count of a netlist — the size driver for the
/// method auto-selection below.
...
pub fn solve_full(
    ...
    for p in &nl.ports {
        let a = nl
            .node_idx(&p.n1)
            .ok_or_else(|| SolveError::UnknownNode(p.n1.clone()))?;
        let c = nl
            .node_idx(&p.n2)
            .ok_or_else(|| SolveError::UnknownNode(p.n2.clone()))?;
        port_pairs.push((a, c));
    }
    ...
    for &f in &freqs {
        ...
        let lu = match method {
            Method::Direct => {
                Some(LuDecomposition::factor(&zb).map_err(|_| SolveError::Singular(f))?)
            }
            Method::Iterative => None,
        };
        let bs = 16.min(nb).max(1);
        let iterative = match method {
            Method::Iterative => Some((
                DenseComplexOperator::new(&zb),
        ...
        // Columns are independent -> rayon (indexed collect, order-preserving).
...
/// components. Component ids are compacted in ascending node order, never a
/// map iteration.
