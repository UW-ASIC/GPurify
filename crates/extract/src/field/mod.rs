...
// The ported FastHenry-stack numerics foundation (from main's
// `pex::quasistatic`): CPU-only FP64, no `gpu` feature required.
pub mod constants;
pub mod geometry;
pub mod henry;
pub mod integrals;
...
pub fn extract_into(
    ...
    debug_assert!(
        ...
        "a tolerance of zero is unreachable"
    );
    debug_assert!(
        options.max_iterations > 0,
        "a solve with no iteration budget"
    );
    ...
    #[cfg(feature = "gpu")]
    let (residual, iterations, backend) = {
    ...
    // Without the feature there is no device to probe: the host `f64` adapter is
    // both operators, which is exactly the fallback the probe lands on anyway.
    #[cfg(not(feature = "gpu"))]
    let (residual, iterations, backend) = columns_into(&host, &host, &mesh, options, matrix)?;
...
fn columns_into<A: matvec::MatVec, F: matvec::MatVec>(
    ...
    for column in 0..n {
        ...
        for row in 0..n {
            ...
            debug_assert!(
                start <= end && end <= panels,
                "band {row} is {start}..{end}"
            );
...
fn mesh_options(
    ...
    debug_assert!(
        feature >= 1,
        "a resolution of zero is an infinite refinement"
    );
