...
fn dc_resistance_is_exact() {
    ...
    let res = gpurify_extract::field::henry::run(&deck).unwrap();
...
fn subdivided_inductance_close_to_single_bar() {
    ...
    let f = 1.0e3;
    let single = {
        let res = gpurify_extract::field::henry::run(&wire_deck(1, 1, f)).unwrap();
        let w = 2.0 * std::f64::consts::PI * f;
        res.z[0][(0, 0)].im / w
    };
    let sub = {
        let res = gpurify_extract::field::henry::run(&wire_deck(4, 4, f)).unwrap();
        let w = 2.0 * std::f64::consts::PI * f;
        res.z[0][(0, 0)].im / w
    };
...
fn skin_effect_raises_ac_resistance() {
    ...
    let r_dc = gpurify_extract::field::henry::run(&wire_deck(6, 6, 1.0))
        .unwrap()
        .z[0][(0, 0)]
        .re;
    let r_ac = gpurify_extract::field::henry::run(&wire_deck(6, 6, 1.0e9))
        .unwrap()
        .z[0][(0, 0)]
        .re;
    println!(
...
fn equiv_merges_nodes() {
    ...
    let equiv = "\
        ...
        .freq fmin=1e3 fmax=1e3 ndec=1\n\
        .end\n";
    let za = gpurify_extract::field::henry::run(joined).unwrap().z[0][(0, 0)];
    let zb = gpurify_extract::field::henry::run(equiv).unwrap().z[0][(0, 0)];
...
fn ground_plane_reduces_inductance_and_matches_image_loop() {
    ...
    let freq = 1.0e3;
    let ind = |deck: &str| {
        let r = gpurify_extract::field::henry::run(deck).unwrap();
...
fn mesh_formulation_matches_nodal() {
    ...
    for (di, deck) in decks.iter().enumerate() {
        let nl = gpurify_extract::field::henry::parse(deck).unwrap();
        let zn = gpurify_extract::field::henry::solve_full(
            &nl,
            gpurify_extract::field::henry::Method::Direct,
            gpurify_extract::field::henry::Formulation::Nodal,
        )
        .unwrap();
        let zm = gpurify_extract::field::henry::solve_full(
            &nl,
            gpurify_extract::field::henry::Method::Direct,
            gpurify_extract::field::henry::Formulation::Mesh,
        )
        .unwrap();
    ...
    fn solve_bits(threads: usize) -> Vec<(u64, u64)> {
        ...
        pool.install(|| {
            let deck = "\
                ...
                .end\n";
            let nl = gpurify_extract::field::henry::parse(deck).unwrap();
            let res = gpurify_extract::field::henry::solve_with(
                &nl,
