...
use gpurify_check::erc::rules::reliability::{
...
struct Powered {
    grid: gpurify_check::erc::power::PowerGrid,
    solution: gpurify_check::erc::power::PowerSolution,
}
...
fn hv_domain_skips_without_intent_and_flags_the_straddling_device_with_it() {
    ...
    assert_eq!(
        ...
        gpurify_check::report::Measurement::Voltage(millivolts(3_300.0)),
        "the measurement is the widest nominal spread across the device's terminals"
    );
    assert_eq!(
        violations.limit[0],
        gpurify_check::report::Measurement::Voltage(millivolts(1_000.0))
    );
